import logging
import sqlite3
from ExpiryService.db.context import PostgresqlCursorContextManager, PostgresqlConnectionContextManager, \
...
class DBConnector:
    ...
    def connect_psycopg(cls, host, port, username, password, dbname, minConn=1, maxConn=10):
        ...
        try:
            if is_psycopg2_importable:
                ...
                logging.getLogger('ExpiryService').info("Connect to {} as user {} to database {}".format(host, username, dbname))
                return True
            ...
            logging.getLogger('ExpiryService').error('Could not connect to ThreadedConnectionPool: {}'.format(e))
            return False
    ...
    def connect_sqlite(cls, path):
        ...
        try:
            ...
            logging.getLogger('ExpiryService').error('Could not connect to sqlite Database: {}'.format(ex))
            return False
